...
inStrings = string.split()
print inStrings
matrix = [[0 for row in range(size)] for col in range(size)]

for row in range(size):
    for col in range(size):
	matrix[row][col] = int(inStrings[row * size + col])

for row in range(size):
    for col in range(size + 1 - pSize):
	p1 = 1
	p2 = 1
...
for row in range(size - pSize + 1):
    for col in range(size - pSize + 1):
	p = 1
	for i in range(pSize):
